/* The game event functions should be split up into th e */
...
function Calendar(props) {
  ...
  function changeCurrentDay(day, currentDays) {
    let dayToSimTo = day.date;
    let tempDate = new Date(currentDay);
    while(tempDate.getTime() < dayToSimTo.getTime()) {
        if(tempDate.getTime() >= playoffDate.getTime()) {
          let tempDayData = currentDays.find((currentDay) => currentDay.date.getTime() === tempDate.getTime())
          if(tempDayData.playoffGame) calculatePlayoffOutcome();
        }
        else {
  ...
  function setPlayoffBracket(playoffBracket) {
    console.log('setting playoffs');
    console.log(playoffBracket);
    setPlayoffRecords(playoffBracket);
  }

  function pushPlayoffWinners(team) {
    let currentWinners = playoffWinners;
    let tempTeam = team;
    tempTeam.seriesRecord.wins = 0;
    tempTeam.seriesRecord.losses = 0;

    currentWinners.push(tempTeam);
    console.log('Winner of series: ', team)
    setPlayoffWinners(currentWinners);
    if(currentWinners.length === playoffRecords.length / 2) {
      /* Here is where I need to set the logic for all of the next round calculations */
      setNextRound()
    }
  }
  ...
  function calculatePlayoffOutcome() {
    let tempGames = [];
    for(let i = 0; i < playoffRecords.length; i++) {
      let currentGame = playoffRecords[i];
      let homeGoals = 0;
      let awayGoals = 0;
      let homeTeamOverall = utils.calculateTeamRating(currentGame.home.team)
      let awayTeamOverall = utils.calculateTeamRating(currentGame.away.team)
      let gameFactor;
      if(homeTeamOverall > awayTeamOverall) {
          let homeOdds = .5 + ((homeTeamOverall - awayTeamOverall) / 100)
          let awayOdds = 1 - homeOdds;
      ...
      if(generateWeightedNumber(gameFactor) === 0) {
          ...
          currentGame.home.seriesRecord.wins = currentGame.home.seriesRecord.wins + 1;
          currentGame.away.seriesRecord.losses = currentGame.away.seriesRecord.losses + 1;
      }
      else {
          awayGoals = getRandomInt(1, 10);
          if(awayGoals > 1) {
          ...
          currentGame.away.seriesRecord.wins = currentGame.away.seriesRecord.wins + 1;
          currentGame.home.seriesRecord.losses = currentGame.home.seriesRecord.losses + 1;
      }
      if(currentGame.home.seriesRecord.wins === 4) {
        pushPlayoffWinners(currentGame.home);
      } else if(currentGame.away.seriesRecord.wins === 4) {
        pushPlayoffWinners(currentGame.away);
      }
      tempGames.push(currentGame);
    }
    setPlayoffBracket(tempGames);
  }
  ...
  return(<div className="calendar">
      ...
      <div className='cal'>
          <CalendarDays day={currentDay} myTeam={myTeam} teamHolder={teamHolder} playoffBracket={playoffRecords} setPlayoffBracket={setPlayoffBracket} changeCurrentDay={changeCurrentDay}/>
      </div>
  </div>)
}
